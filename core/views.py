...
def index(request):
    if request.method == "POST":
        ...
        if form_type == "session_finder":
            session_finder_form = SessionFinderForm(request.POST)
            if session_finder_form.is_valid():
                return redirect('core:index_session',
                                session_slug_name=session_finder_form.cleaned_data['session_slug_name'])
    else:
        session_finder_form = SessionFinderForm()

    return render(request, os.path.join('core', 'index.html'), locals())
    ...
    if request.user.is_authenticated and request.user.is_staff:
        if request.method == "POST":
            ...
            if create_session_form.is_valid():
                session_group = Group.objects.create(name=SESSION_GROUP_PREFIX +
            ...
            create_session_form = CreateSessionForm()
    return render(request, os.path.join('core', 'create_session.html'), locals())
        ...
        if session.can_register:
            if request.method == "POST":
                ...
                if form_type == "registration_form":
                    registration_form = SessionPlayerRegistration(request.POST, session=session)
        ...
        if request.method == "POST":
            ...
            if form_type == "login_form":
                ...
                if login_form.is_valid():
                    user = authenticate(username=login_form.cleaned_data["player"].user.username,
            ...
            if form_type == "guest_form":
                ...
                if guest_form.is_valid():
                    ...
                    authenticated_user = authenticate(username=user.username,
                                                      password=guest_password(user.username))
    ...
    if admin_user:
        ...
        if request.method == "POST":
            ...
            if form_type == "modify_session_form":
                ...
                if modify_session_form.is_valid():
                    ...
                    session.need_registration = modify_session_form.cleaned_data["need_registration"]
        ...
        if request.method == "POST":
            ...
            if form_type == "add_player_form":
                add_player_form = SessionPlayerRegistration(request.POST, session=session)
        ...
        players = Player.objects.filter(session=session, is_guest=False).exclude(user__id__in=session.admins.all())
        ...
        if request.method == "POST" and request.POST["form_type"] == "update_password_form":
            player_id = int(request.POST["update_password_player_id"])
            for player in players:
                if player.id == player_id:
                    update_password_form = UpdatePasswordForm(request.POST, session=session)
                    update_password_forms[player] = update_password_form
                    if update_password_form.is_valid():
                        player.user.set_password(update_password_form.cleaned_data["password2"])
                        player.user.save()
        ...
        for game in games:
            if request.method == "POST":
                ...
                if form_type == "modify_game_form_" + str(game.url_tag):
                    modify_game_form = ModifyGameForm(request.POST, session=session, game=game, prefix=game.url_tag)
                    if modify_game_form.is_valid():
                        ...
                        modify_game_form = ModifyGameForm(session=session, game=game, prefix=game.url_tag)
