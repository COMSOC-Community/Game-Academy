...
from .random import assign_random_questions
...
class Index(GameIndexView):
    def get(self, request, session_url_tag, game_url_tag):
        if (
            self.context["answer"]
            and not self.context["answer"].question_answers.exists()
        ):
    ...
    def get(self, request, session_url_tag, game_url_tag):
        ...
        if submitting_player:
            if not self.context["answer"]:
                ...
                assign_random_questions(game, answer)
        ...
        return render(
            request, os.path.join("goodbad", "submit_answer.html"), self.context
        )
    ...
    if game_result:
        ...
        if answer:
            ...
            for question in answer.questions.all():
                question_answer = answer.question_answers.filter(
                    question=question
                ).first()
                question_result = question.results.filter(result=game_result).first()
                questions_answer_result.append(
                    (question, question_answer, question_result)
                )
            context["questions_answer_result"] = questions_answer_result
