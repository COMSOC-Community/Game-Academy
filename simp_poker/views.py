...
class SubmitAnswer(GameSubmitAnswerView):
    def get(self, request, session_url_tag, game_url_tag):
        ...
        return render(
            request, os.path.join("simp_poker", "submit_answer.html"), self.context
        )
...
class Results(GameResultsView):
    def get(self, request, session_url_tag, game_url_tag):
        ...
        context["answers_sorted_round_robin_with_opt"] = answers.order_by(
            "-round_robin_with_opt_score"
        )
        context["answers_sorted_against_opt"] = answers.order_by(
        ...
        if round_robin_winners:
            if len(round_robin_winners) == 1:
                context[
                    "formatted_round_robin_winners"
                ] = round_robin_winners.first().player.display_name()
            else:
                winners = [
                    f"<em>{a.player.display_name()}</em>" for a in round_robin_winners
                ]
                context["formatted_round_robin_winners"] = (
                    ", ".join(winners[:-1]) + " and " + winners[-1]
                )
        ...
        if winners_agains_opt:
            if len(winners_agains_opt) == 1:
                context[
                    "formatted_winners_against_opt"
                ] = winners_agains_opt.first().player.display_name()
            else:
                winners = [
                    f"<em>{a.player.display_name()}</em>" for a in winners_agains_opt
                ]
                context["formatted_winners_against_opt"] = (
                    ", ".join(winners[:-1]) + " and " + winners[-1]
                )
