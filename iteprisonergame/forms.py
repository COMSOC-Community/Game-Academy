...
class SubmitAnswerForm(forms.Form):
    name = forms.CharField(label="Name of the Strategy",
                           label_suffix="")
    initial_state = forms.CharField(label="Initial State",
                                    label_suffix="",
    ...
    def clean_automata(self):
        ...
        for line_index in range(len(lines)):
            ...
            if match:
                ...
                if state in automata.transitions and "C" in automata.transitions[state] and "D" in automata.transitions[state]:
                    errors.append("Line {} redefines state {}.".format(line_index + 1, state))
    ...
    def clean(self):
        ...
        if "automata" in cleaned_data:
            ...
            if not found_initial_state:
                raise forms.ValidationError("The initial state is not part of the states described in the strategy.")
        return cleaned_data
